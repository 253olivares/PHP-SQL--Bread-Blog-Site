 ...
 * Date: 4/28/2020
 * File: bread_recipe_detail.class.php
 * Description:
 */

class BreadRecipeDetail extends BreadNav
{
            ...
            if ($userID == 5 || $userID == $authorID) {
                ...
                <div class="editButton">
                    <input style="float: left" type="button" id="Edit" value="  Edit  "
                           onclick="window.location.href = '<?= BASE_URL ?>/bread/edit/<?= $id ?>'">
                    <input style="float: right" type="button" id="Delete" value="  Delete  "
                           onclick="  var r = confirm('Are u sure you want to delete the recipe'); if(r == true){window.open('<?= BASE_URL ?>/bread/delete/<?= $id ?>/','_self')}">
                </div>
