...
class BreadModel
{
    ...
    //creates one instance of bread model
    public static function getBreadModel()
    {
        if (self::$_instance == NULL) {
            self::$_instance = new BreadModel();
        }
        return self::$_instance;
    }

    //function that lists all recipes
    public function listBreadRecipes()
    {
        //our try here check to make sure our sql works

        try {

        $sql = "SELECT * FROM " . $this->tblRecipes . " WHERE " . $this->tblRecipes . ".Category= 'Bread'";
...
//        $sql = "SELECT * FROM submissions,users WHERE Submission_id = $id  AND submissions.id=users.id";
    ...
    public function searchRecipe($terms)
    {
        ...
        $sql = "SELECT * FROM " . $this->tblRecipes . " WHERE " . $this->tblRecipes . ".Category= 'Bread' AND " . $this->tblRecipes . ".id=" . $this->tblRecipes . ".id AND (1";
