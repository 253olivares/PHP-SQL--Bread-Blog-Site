 ...
 * Date: 4/20/2020
 * File: cake_recipe_detail.class.php
 * Description: this is the detail page to display a recipes information
 */

class CakeRecipeDetail extends CakeNav
{
            ...
            if ($userID == 5 || $userID == $authorID) {
                ...
                <div class="editButton">
                    <input style="float: left" type="button" id="Edit" value="  Edit  "
                           onclick="window.location.href = '<?= BASE_URL ?>/cake/edit/<?= $id ?>'">
                    <input style="float: right" type="button" id="Delete" value="  Delete  "
                           onclick="  var r = confirm('Are u sure you want to delete the recipe'); if(r == true){window.open('<?= BASE_URL ?>/cake/delete/<?= $id ?>/','_self')}">
                </div>
